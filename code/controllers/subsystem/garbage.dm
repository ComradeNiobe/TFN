...
/datum/controller/subsystem/garbage/proc/HandleQueue(level = GC_QUEUE_FILTER)
	...
	for (var/i in 1 to length(queue))
		...
		if (IS_DELETED(D, GCd_at_time)) // So if something else coincidently gets the same ref, it's not deleted by mistake
			...
			pass_counts[level]++
			#ifdef REFERENCE_TRACKING
			reference_find_on_fail -= FAST_REF(D) //It's deleted we don't care anymore.
			#endif
			if (MC_TICK_CHECK)
				return
			continue
		...
		switch (level)
			if (GC_QUEUE_CHECK)
				#ifdef REFERENCE_TRACKING
				if(reference_find_on_fail[FAST_REF(D)])
					...
					ref_searching = TRUE
				#endif
				reference_find_on_fail -= FAST_REF(D)
				#endif
				var/type = D.type
				var/datum/qdel_item/I = items[type]

				var/message = "## TESTING: GC: -- [FAST_REF(D)] | [type] was unable to be GC'd --"
#if DM_VERSION >= 515
